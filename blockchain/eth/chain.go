...
// NewChain returns a new chain state struct for usage
...
func (c *Chain) AddBlock(b *BlockInfo, source BlockSource) int {
	...
	// update metadata if block is already stored, otherwise update all block info
	if c.HasBlock(hash) {
	...
	// if source is BDN, then no authority to update chain state and indicate no new heads to return
	if source == BSBDN {
		return 0
	}
...
func (c *Chain) updateChainState(height uint64, hash ethcommon.Hash, parentHash ethcommon.Hash) int {
	...
	// build chain state from previous head to the latest block
	...
	for ; headHeight > chainHead.height; headHeight-- {
		...
		if !ok {
			// TODO: log anything? chain state can't be reconciled (maybe should just prune to head)
			return 0
		}
	...
	// chain state was successfully reconciled (some entries were just missing), nothing else needed
	...
	for ; i < len(c.chainState); i++ {
		...
		// exceeded c.maxReorg, trim the chain state
		if i+1 >= c.maxReorg {
			c.chainState = missingEntries
			return len(missingEntries)
		}
...
func (c *Chain) getBlockMetadata(hash ethcommon.Hash) (blockMetadata, bool) {
	bm, ok := c.blockHashToMetadata.Get(hash.String())
	if !ok {
